...
if 'app' not in st.session_state:
    st.session_state.app = None
if 'camera_active' not in st.session_state:
    st.session_state.camera_active = False # Controls visibility of the WebRTC component
...
def main():
    ...
    st.markdown("""
    Phần demo nhận diện khuôn mặt của nhóm sử dụng **độ lỗi ArcFace**, được thực hiện bằng cách gọi mô hình `buffalo_l` — một mô hình đã được huấn luyện bằng hàm **ArcFace Loss**.
    - **Detection Model**: RetinaFace-10GF | **Recognition Model**: ResNet50@WebFace600K | **Model Size**: 326MB  
    """)
            ...
            try:
                ...
                if not cap.isOpened():
                    ...
                    while cap.isOpened():
                        ...
                        frame_count += 1
                        progress_percentage = int((frame_count / total_frames) * 100) if total_frames > 0 else 0
                        progress_bar.progress(progress_percentage)
                        status_text.text(f"Đang xử lý frame: {frame_count}/{total_frames}")
    ...
    with tab_live:
        st.header("Nhận diện khuôn mặt thời gian thực (WebRTC)")
        ...
        if st.session_state.camera_active:
            st.info("Nhấn 'START' để bắt đầu truyền video từ camera của bạn.")

            # RTC Configuration for deployment
            rtc_config = RTCConfiguration({"iceServers": [{"urls": ["stun:stun.l.google.com:19302"]}]})

            def webrtc_callback(frame: av.VideoFrame) -> av.VideoFrame:
                # Convert the frame to a NumPy array for processing
                img = frame.to_ndarray(format="bgr24")

                # Process the frame using your existing logic
                processed_img = process_frame(img, st.session_state.app, st.session_state.face_db)
                
                # Convert the processed NumPy array back to a VideoFrame
                return av.VideoFrame.from_ndarray(processed_img, format="bgr24")

            webrtc_streamer(
                key="webrtc_streamer_component",
    ...
    with tab_manage:
        ...
        st.subheader("Thông tin hệ thống")
        status_icon = '🟢' if st.session_state.camera_active else '🔴'
        status_text = 'Component đang hiển thị' if st.session_state.camera_active else 'Component đang ẩn'
        st.write(f"• **Trạng thái WebRTC Component:** {status_icon} {status_text}")
